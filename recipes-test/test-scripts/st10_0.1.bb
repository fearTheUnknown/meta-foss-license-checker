...
SRC_URI = "file://strong_weak_executable.c"
...
do_compile(){
    ${CC} -DUSE_SYSCALL strong_weak_executable.c ${LDFLAGS} -o strong_weak_executable -lstrong_weak
}

do_install(){
    install -d ${D}${bindir}
    install -m 0755 strong_weak_executable ${D}${bindir}
}
