...
def package_qa_check_license_compliance(packages, pkgfiles, d):
    ...
    recipe_sysroot_dir = m_datastore.getVar('RECIPE_SYSROOT')
    ...
    if is_executables_generated:
        ...
        for raw_linked_lib_name in raw_linked_lib_names:
            ...
            #If there are more than 2 lib file matching the same name of a linked lib
                #Check if compile log has pattern flag -static
                    #If yes, then add the .a lib into linked libs
                    #Set linking attribute to static
                
                #else 
                    #Add the .so lib into linked libs
                    #Set linking attribute to dynamic
